...
# The full suite, STDLIB-ONLY -- the same 92 files as CLAUDE.md's canonical
# command and as the CI workflow's first step, so the three cannot disagree
# about what "all the tests" means. They did for eight days: CLAUDE.md listed
# 92, CI listed 78 and this file listed 41, and the 51 missing here included
# the whole approvals security suite.
#
# `test_x402_pay.py` belongs in this list even though it needs eth-account,
# because clients/x402_pay.py skips cleanly without it. It used to SystemExit(2)
# at import, which killed the entire invocation before any test ran -- so its
# presence here is a live check that the skip still works.
test:
	python3 -m unittest \
	  ...
	  test_seller_audit.py test_seller_portal.py test_seller_report.py \
	  test_settlement_sim.py test_settlement_velocity.py test_settlement_watch.py \
	  test_solana_rwa.py test_token_decimals.py test_tokenized_stock_registry.py \
...
# and native backends must be byte-identical, so running it one way proves half
