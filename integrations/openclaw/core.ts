/**
 * blackwall-openclaw-gate CORE -- every part of this plugin that does NOT need
 * the OpenClaw host.
 *
 * SPLIT OUT 2026-09-15. This file's contents lived in `index.ts` alongside
 * `import { definePluginEntry } from "openclaw/plugin-sdk/plugin-entry"`, which
 * made the whole module unimportable from anywhere that is not an OpenClaw
 * plugin -- one host-only import held the CLAIM PARSING and the GO/HOLD/STOP
 * DECISION hostage. It surfaced when `integrations/lucid` tried to reuse
 * `decide`: the alternative was a second copy of the decision, and two copies
 * of "what does a verdict mean" is exactly the drift this repo keeps paying
 * for.
 *
 * This is the shape every other integration here already has and this one
 * did not -- `blackwall_guard.py` is LangChain-free with a thin
 * `langchain_blackwall.py` over it, `wallet_guard.py` is provider-free with
 * thin Turnkey/Privy shims, `agentcore_guard.py` is dependency-free with thin
 * Strands/LangGraph adapters. `index.ts` is now that thin adapter and
