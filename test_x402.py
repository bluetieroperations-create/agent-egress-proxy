...
class TestBillingGate(unittest.TestCase):
    ...
    def test_settle_payload_carries_resource_for_bazaar(self):
        # CDP Bazaar indexes the PAID endpoint from paymentPayload.resource in the
        # verify/settle call (x402 v2 moved `resource` out of paymentRequirements).
        # The gate must inject its CANONICAL endpoint URL so a CDP settlement
        # catalogs THIS service -- and it must be that URL, NOT the agent-supplied
        # `resource` arg (which is what the agent is paying ABOUT).
        # Kills: not injecting resource -> settled forever but never listed.
        seen = {}
        RES = "https://agent-egress-proxy.onrender.com/v1/forecast-payment"

        class _Cap(X.MockFacilitator):
            def verify(self, payment, req):
                seen["verify"] = dict(payment)
                return super().verify(payment, req)

            def settle(self, payment, req):
                seen["settle"] = dict(payment)
                return super().settle(payment, req)

        cfg = X.BillingConfig(price="0.001", pay_to=PAY_TO, resource_url=RES)
        gate = X.BillingGate(cfg, facilitator=_Cap(approve=True))
        r = gate.check("https://agent-resource.example/run",
                       x_payment=encode(make_payment()))
        self.assertTrue(r.paid)
        # Present on BOTH verify and settle (CDP needs it on the settle call).
        self.assertEqual(seen["verify"]["resource"], RES)
        self.assertEqual(seen["settle"]["resource"], RES)

    def test_no_resource_url_leaves_payload_untouched(self):
        # Backward compat: no resource_url configured -> no injection, so nothing
        # that previously worked (community facilitators, tests) changes.
        seen = {}
        ...
        gate = X.BillingGate(X.BillingConfig(price="0.001", pay_to=PAY_TO),
                             facilitator=_Cap(approve=True))
        gate.check("https://r", x_payment=encode(make_payment()))
        self.assertNotIn("resource", seen["settle"])

    def test_client_supplied_resource_is_overwritten_not_deferred_to(self):
        # SECURITY: CDP catalogs paymentPayload.resource, so whoever controls that
        # field controls what endpoint gets listed for OUR settlement. An attacker
        # can put ANY top-level `resource` in their X-PAYMENT; our injection MUST
        # overwrite it with the operator-configured canonical URL, never defer to
        # the client value. Kills the `setdefault(...)`/`.get("resource") or ...`
        # mutant, under which an attacker's resource would reach CDP unchanged on
        # BOTH verify and settle.
        seen = {}
        RES = "https://agent-egress-proxy.onrender.com/v1/forecast-payment"
        ATTACKER = "https://evil.example/attacker-catalog"
        ...
        payment["resource"] = ATTACKER  # attacker-controlled X-PAYMENT field
        cfg = X.BillingConfig(price="0.001", pay_to=PAY_TO, resource_url=RES)
        gate = X.BillingGate(cfg, facilitator=_Cap(approve=True))
        r = gate.check("https://agent-resource.example/run",
                       x_payment=encode(payment))
        self.assertTrue(r.paid)
        # The attacker's value must be gone on both facilitator calls.
        self.assertEqual(seen["verify"]["resource"], RES)
        self.assertEqual(seen["settle"]["resource"], RES)
        self.assertNotEqual(seen["settle"]["resource"], ATTACKER)
